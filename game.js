...
if(event.key === "Escape"){
if(!this.isPaused){ this.pauseGame(); } else { this.unpauseGame(); }
return;
}
...
const key = event.key;
let selected = null;
for(const up of this.upgradeBoxes){ if(up.key === key){ selected = up.type; break; } }
...
count--;
if(count>0){ cText.setText(count.toString()); }
else { cText.destroy(); this.isPaused = false; }
}});
}


// ===== Combat/UI helpers =====
playerAttack(){ this.tweens.add({ targets:this.player, scaleX:2.2, scaleY:2.2, duration:150, yoyo:true, ease:'Power2' }); }
...
enemyTakeDamage(){ this.tweens.add({ targets:this.enemy, tint:0xff0000, duration:200, yoyo:true, onComplete: ()=> this.enemy.clearTint() }); }
...
playerTakeDamage(){ this.tweens.add({ targets:this.player, tint:0xff0000, duration:300, yoyo:true, onComplete: ()=> this.player.clearTint() }); }
...
if(this.finalScore > this.personalBest){ this.personalBest = this.finalScore; personalBest = this.personalBest; }
}
create(){
